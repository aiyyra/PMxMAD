...
class AppointmentNoteRepository {
    ...
    private AppointmentNoteDao mNoteDao;
    private LiveData<List<AppointmentNote>> mAllNotes;
    ...
    AppointmentNoteRepository(Application application) {
        AppointmentNoteRoomDatabase db = AppointmentNoteRoomDatabase.getDatabase(application);
        mNoteDao = db.noteDao();
    ...
    LiveData<List<AppointmentNote>> getAllNotes() {
        return mAllNotes;
    }
    ...
    void insert(AppointmentNote note) {
        AppointmentNoteRoomDatabase.databaseWriteExecutor.execute(() -> {
